...
let valves = input_converter_1.input_converter.get_valves(input);
// Task 1:
...
let initial_valve = valves.find((valve) => valve.name === "AA");
// Get a list of all valves with a value.
let valves_with_flow_rate = valves.filter((v) => v.flow_rate > 0);
...
for (let i = 0; i < valves_with_flow_rate.length; ++i) {
    for (let j = 0; j < valves_with_flow_rate.length; ++j) {
        let x = valves_with_flow_rate[i];
        let y = valves_with_flow_rate[j];
        if (x.name == y.name) {
            continue;
        }
        if (!(x.name in y.valve_steps)) {
            let fewest_required_steps = 999999999;
            function count_steps(a, b, steps = 0) {
                if (steps == 15) {
                    return;
                }
                if (a.name == b.name) {
...
let permutations = [];
function permute(arr, m, cur_steps = 0) {
    if (arr.length === 0) {
        permutations.push(m);
    }
    else {
...
for (let i = 0; i < permutations.length; ++i) {
    ...
    for (let m = 1; m <= total_minutes; ++m) {
        ...
        let next_steps = path.length == path_idx + 1
            ? 999999999
        ...
        if (next_steps == turns_on_current_step) {
            ...
            valves_with_flow_rate.find((v) => v.name == path[path_idx + 1].name).is_open = true;
            path_idx += 1;
    ...
    totals.push([total_flow_rate, permutations[i].map((x) => x.name)]);
}
let outer_max = 100;
let inner_max = 1000000;
let highest_totals = totals
    .sort((a, b) => b[0] - a[0])
    .slice(0, inner_max);
let best_combination_total = 0;
for (let i = 0; i < outer_max; ++i) {
    for (let j = 0; j < highest_totals.length; ++j) {
        if (highest_totals[i][1].some((x) => highest_totals[j][1].includes(x))) {
            continue;
        }
        let combination_total = highest_totals[i][0] + highest_totals[j][0];
